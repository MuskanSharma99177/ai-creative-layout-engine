...
function ensureAccessibleTextColor(bgHex: string, preferredTextHex: string): string {
  try {
    ...
    // Fall back to high contrast dark or light
...
export function generateFallbackLayout(input: CreativeInputDto): LayoutConfigDto {
  ...
  const isQuickConversion =
    goalLower.includes('conversion') ||
    goalLower.includes('sales') ||
    isSaleOrDiscount;

  // Decision Tree for Layout Archetype
  let layoutType: LayoutConfigDto['layoutType'] = 'image-left-content-right';
  let visualEmphasis: LayoutConfigDto['composition']['visualEmphasis'] = 'product-image';
  let fontFamily: LayoutConfigDto['theme']['fontFamily'] = 'sans';
  let headlineScale: LayoutConfigDto['typography']['headlineScale'] = 'large';
  let ctaStyle: LayoutConfigDto['composition']['ctaStyle'] = 'solid';
  let imagePosition: LayoutConfigDto['composition']['imagePosition'] = 'left';
  let spacing: LayoutConfigDto['composition']['spacing'] = 'comfortable';
  ...
  if (isLuxury) {
    ...
    visualEmphasis = isSaleOrDiscount ? 'discount-badge' : 'product-image';
    headlineScale = 'heroic';
    ctaStyle = 'gradient';
    imagePosition = 'right';
    spacing = 'compact';
  } else if (descLen > 160) {
    // Text-heavy layout needed
